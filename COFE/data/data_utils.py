...
def flatten_nli2(example):
    ...
    return {"source": prefix + example['source'], "target": example['target'], "id": example["id"],
            'task': example['task'], 'nli': nli_prefix}


def flatten_nli(example):
    return {"source": example['premise'], "target": example['hypothesis'], "label": example["label"],
            "id": example["id"], 'task': example['task']}


def nli_add_index_and_task(example, index):
    example.update({"id": index})
    example.update({"task": "aux"})
    return example


def preprocessing_nli(nli_dataset_dict: datasets.DatasetDict):
    nli_dataset_dict = nli_dataset_dict.map(nli_add_index_and_task, with_indices=True, num_proc=4)
    nli_columns = nli_dataset_dict["train"].column_names
    nli_dataset_dict = nli_dataset_dict.map(function=flatten_nli, remove_columns=nli_columns, num_proc=4)
    nli_columns = nli_dataset_dict["train"].column_names
    nli_dataset_dict = nli_dataset_dict.map(function=flatten_nli2, remove_columns=nli_columns, num_proc=1)
    return nli_dataset_dict


def add_control_code_doc(example, index, control_code: str = "", text_column: str = None, summary_column: str = None):
    prefix = f"[{control_code}] "

    return {"document": prefix + str(example[text_column]), "summary": example[summary_column],
            "index": index}
