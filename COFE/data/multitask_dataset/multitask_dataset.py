...
from collections import OrderedDict as ordereddict, defaultdict
...
def random_choice_in_dataset(rng, dataset, choice_size):
...
def _clean_if_not_none(var_list):
    for v in var_list:
        if v is not None:
            del v
...
class SampledMultiDataset(BaseWrapperDataset):
    ...
    Args:
        ...
        sampling_ratios (OrderedDict[str, ~torch.utils.data.Dataset]): list of probability of each dataset to be sampled
    ...
    def __init__(
            self,
            datasets: OrderedDict[str, BaseWrapperDataset],
            sampling_ratios=Union[None, OrderedDict[str, int]],
            seed=42,
            epoch=0,
            virtual_size: Union[Callable, int] = default_virtual_size_func,
            shuffle=True,
    ):
        super().__init__()
        ...
        if not isinstance(datasets, OrderedDict):
            raise AssertionError()
        self.keys = list(datasets.keys())
        datasets = list(datasets.values())
        self.datasets = datasets
        if sampling_ratios is not None:
            sampling_ratios_keys = list(sampling_ratios.keys())
            assert sampling_ratios_keys == self.keys
            sampling_ratios = list(r for k, r in sampling_ratios.items())
    ...
    def _reset_cached_properties(self):
        _clean_if_not_none([self._sizes, self._cur_indices])
        self._sizes = None
        self._cur_indices = None

    def setup_sampling(self,
                       sample_ratios: Union[None, np.ndarray, List[int]],
                       virtual_size: Union[Callable, int]):
        sizes = [len(d) for d in self.datasets]
        if sample_ratios is None:
            # default back to concatenating datasets
            self.sample_ratios = None
    ...
    def get_virtual_indices(self, rng: np.random.RandomState, datasets, sample_ratios, virtual_size):
        """
        establish virtual

        :param rng: random generator
        :param datasets: from self.datasets
        :param sample_ratios: from self.sample_ratios
        :param virtual_size: from self.virtual_size
        :return:
        indices: 1D
        cumulated_sizes: 1D
        virtual_size_per_dataset 1D
        """

        def get_counts(in_count_sample_ratios):
            counts = np.array([virtual_size * r for r in in_count_sample_ratios], dtype=np.int64)
            diff = virtual_size - counts.sum()
            assert diff >= 0
            # due to round-offs, the size might not match the desired sizes
            if diff > 0:
                dataset_indices = rng.choice(
                    len(in_count_sample_ratios), size=diff, p=in_count_sample_ratios
                )

                # randomly add 1 to some datasets,
                # make the final diff<0
        ...
        def get_in_dataset_indices(datasets, sizes, sample_ratios):
            ...
            indices = [
                random_choice_in_dataset(rng, d, c)
    ...
    def _get_dataset_and_index(self, index):
        ...
        return int(i), int(self._cur_indices[index])
    ...
    def _establish_virtual_datasets(self):
        ...
        indices, cumulated_sizes, virtual_size_per_dataset = self.get_virtual_indices(rng, self.datasets,
                                                                                      self.sample_ratios,
                                                                                      self.virtual_size)
        ...
        logger.info(
            f" Raw sizes: {str(dict(zip(self.keys, raw_sizes)))}; "
            f"raw total size: {sum(raw_sizes)}"
        )
        logger.info(
            f" Resampled sizes: {str(dict(zip(self.keys, sampled_sizes)))}; "
            f"resampled total size: {sum(sampled_sizes)}"
        )
        if self.sample_ratios is not None:
            logger.info(
                f"Upsampling ratios: {str(dict(zip(self.keys, self.sample_ratios)))}"
            )
        else:
            logger.info(f"[{self.split}] A concat dataset")
        logger.info(
            f" virtual dataset established time: {get_time_gap(start_time, time.time())}"
        )
